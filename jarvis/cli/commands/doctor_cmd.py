...
def run_doctor(gateway):
    ...
    table.add_row("Python", "Version Check", f"✅ {python_ver}")
    
    # 2. Config
    if os.path.exists(gateway._config_path):
        table.add_row("Configuration", "File Existence", f"✅ {os.path.basename(gateway._config_path)}")
    else:
        table.add_row("Configuration", "File Existence", "❌ Missing!")
        
    # 3. LLM Router / Backends
    if gateway.router:
        health = gateway.router.status()
        p_name = gateway.router._primary.name
        res = "✅ Healthy" if health.get(p_name) else "❌ Down"
        table.add_row("LLM Backend", f"Primary ({p_name})", res)
    else:
        table.add_row("LLM Backend", "Router Init", "❌ Failed")
    ...
    if gateway.memory:
        ...
        if os.path.exists(db_path):
            ...
            table.add_row("Memory DB", "Connectivity", f"✅ Connected ({size} KB)")
        else:
            table.add_row("Memory DB", "Connectivity", "❌ Missing file")
    else:
        table.add_row("Memory DB", "Initialization", "❌ Failed")
        
    # 5. External Tools
    ollama_path = shutil.which("ollama")
    if ollama_path:
        table.add_row("Ollama", "Binary Check", "✅ Installed")
    else:
