...
class IntentSafetyLayer:
    ...
    DISCUSSION_PATTERNS = [
        re.compile(r"^\s*(how\s+(?:do|can|to|should|would)|what\s+is|why\s+does|explain|teach|tell\s+me\s+about|how\s+to\s+use|can\s+you\s+explain|summarize|translate|analyze)\b", re.I),
