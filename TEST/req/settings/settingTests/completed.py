...
def open_gaming_setting(setting):
    ...
    settings_map = {
        ...
        "gameingr": "ms-settings:gaming-gamebar",
        "gameing": "ms-settings:gaming-gamebar",
        "gamimg": "ms-settings:gaming-gamebar",
        "game": "ms-settings:gaming-gamebar",
...
def MainSettings(multiVals=[""], addrs=""):
    ...
    if function != None:
        ...
        if len(multiVals) <= 2 or setting_ is not None:
            ...
            # activing Dynamic access
            if multiVals[-1] == "home":
                pass
            pass


def invoke_button_action(window_buttons, userVal):
    # Define mappings from MultVal values to button actions
    if userVal in ['back']:
        for button_properties in window_buttons:
        ...
        for button_properties in window_buttons:
            if button_properties[0] == "Button" and button_properties[1] == 'Maximize Settings':
                button_properties[2].invoke()
                return True
    elif userVal in ['close Settings', 'close']:
        for button_properties in window_buttons:
...
def MainWhatToOpen(MultVal=[""], addr=""):
    # if Val.lower() in ['setting', 'settings']:
    #     open_settings_windows()
    #     return
    MainSettings(MultVal[1:], addrs="")
    ...
    while True:
        ...
        if user in ["", "0", 'exit', 'exit()']:
            ...
            if not invoke_button_action(window_buttons, user.lower()):
                MainWhatToOpen(MultVal)
                update()


if __name__ == '__main__':
    # asyncio.run(bluetooth_on_off(True))
    # open_system_settings("about")
    # if "open setting"
    # MainWhatToOpen("windows update")
    update()
    Infinet()
    pass
