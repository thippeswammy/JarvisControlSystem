...
def get_present_location_in_setting(settings_path=r'C:\Windows\ImmersiveControlPanel\SystemSettings.exe',
    ...
    # Function to find the first "Back" button in the hierarchy
    def find_first_back_button(nested_list):
        element_info, children, depth = nested_list
        if element_info[0] == 'Button' and element_info[1] == 'Back':
            return element_info, depth
        for child in children:
            result = find_first_back_button(child)
            if result:
                return result
        return None
    ...
    back_button_info = find_first_back_button(ui_hierarchy)

    # Print the button elements within the first "Window" of depth 1
    window_buttons_output = []
    if back_button_info:
        back_button, back_depth = back_button_info
        window_buttons_output.append(f"{' ' * back_depth * 4}{back_button[0]}: {back_button[1]} [Depth: {back_depth}]")
    ...
    if buttons:
        ...
        return location_output, location_output_single, buttons, window_buttons_output
    else:
...
if result:
    location_output, location_output_single, buttons, window_buttons_output = result
    print(location_output)
    print(location_output_single)
    # print(window_buttons_output)
